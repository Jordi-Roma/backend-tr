...
PALABRAS_RESERVA = {"reserva", "reservar", "reservas", "carrito"}
...
def responder_chat_asistente(request: AsistenteChatRequest, usuario_actual: dict[str, object] | None) -> AsistenteChatResponse:
    ...
    tipo = _clasificar_intencion(mensaje_normalizado)

    if _requiere_login(mensaje_normalizado) and usuario_actual is None:
    ...
    if tipo == "capacidades":
        return AsistenteChatResponse(respuesta="Puedo ayudarte a buscar prendas reales del catálogo, consultar disponibilidad por talla, color o sucursal, explicarte cómo hacer una reserva, mostrar sucursales y sugerirte alternativas si no hay stock. Por ejemplo: “¿hay poleras rojas talla M?” o “¿dónde está disponible esta prenda?”.", tipo=tipo)

    if tipo == "agradecimiento":
        return AsistenteChatResponse(respuesta="¡De nada! Si quieres, también puedo ayudarte a buscar una prenda, consultar stock o explicarte cómo reservar.", tipo=tipo)
    ...
    if tipo == "ayuda_reservas":
        return AsistenteChatResponse(respuesta="Para hacer una reserva, elige una prenda con stock, selecciona talla/color y sucursal, agrégala al carrito y confirma la reserva. Si quieres ver tus reservas, debes iniciar sesión.", tipo=tipo)
    ...
    respuesta_base = _respuesta_productos(productos, tipo, alternativas)
    respuesta = _redactar_con_ia(mensaje, productos, respuesta_base, tipo)
...
def _requiere_login(mensaje: str) -> bool:
    return any(frase in mensaje for frase in PALABRAS_PERSONALES)
...
def _extraer_precio_maximo(mensaje: str) -> float | None:
    if not any(palabra in mensaje for palabra in ("barato", "barata", "menos de", "maximo", "máximo", "hasta")):
        return None
    match = re.search(r"(?:menos de|hasta|maximo|máximo)\s*(\d+(?:[.,]\d+)?)", mensaje)
...
def _respuesta_productos(productos: list[dict[str, object]], tipo: str, alternativas: list[dict[str, object]] | None = None) -> str:
    if not productos:
        if alternativas:
...
def _redactar_con_ia(mensaje: str, productos: list[dict[str, object]], respuesta_base: str, tipo: str) -> str:
