...
def _crear_embedding(texto: str) -> list[float]:
    ...
    response = client.embeddings.create(model=MODELO, input=texto)
...
def _mensaje_error_embedding(exc: Exception) -> str:
    ...
    if 'connection error' in texto:
        return 'No se pudo conectar con OpenAI para generar embeddings.'
    if exc.__class__.__name__ == 'ModuleNotFoundError':
        return 'La dependencia openai no está instalada en el entorno backend.'
